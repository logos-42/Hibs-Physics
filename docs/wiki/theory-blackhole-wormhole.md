...
> ⚠ **2026-08-14 基础假设修正（leo 指出）**：本页 BH2/BH3 的"向外光子
> dx = (v−c)dt"（光子相对空间以 c 传播）是 Gordon/河流模型表述，**没有
> 完整安装 SLS2 公设（光子 = 完全随空间，相对运动 = 0）**。修正后的
> 公设版（黑洞 = 矢量流场汇，|C|=c 恒，光子 = 流线，逃逸不可能 = 公设
> 直接推论）见 [theory-maxwell-flow.md](./theory-maxwell-flow.md) §0–§3
> （`MaxwellFlow.lean` PH1–PH2）。本页 BH1–BH7 保留为"Gordon 度规代数
> 骨架"（视界=光速面 g_tt=0 ⟺ |v|=c 等度规层结论仍有效），但黑洞的
> 物理图像以公设版为准。
