...
- [theory-anti-gravity-capture.md](./theory-anti-gravity-capture.md) — 反引力场/等效质量消失/旋转流动捕获环（主线 AGC1–AGC6）：反引力=流动偏差抵消 μ=1⟹m=0（攻击锚定 MC1'）；质量消失⟹随流 dτ=0（SM3）；易实现判据（成本∝梯度²、弱场更易）；旋转流动环捕获（Γ≠0⟹无质量随旋流）；磁场是标记非捕获力（FSF5 + m_eff=0）；正/反电子案例（GQP1–4/TW7）
