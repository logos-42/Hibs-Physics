...
- **投影代数(ProjectionAlgebra.lean, PA1–PA6 全部已证)**:
  - PA1 投影 = 幂等自映射(IsProjection);★ 交换投影的复合仍是投影(抽象定理,正交投影族自动满足)
  - PA2 互补投影对 ComplementaryProjection(幂等/正交/完备) = **状态生成机制的代数公理**;ℂ 实例 {Π_re, Π_im}(`realImagProjection`,全部字段已证)
  - PA3 ★ 复合表 5 项全确定:{Π_re, Π_im, 0} 构成半群(变换复合封闭 = 状态生成可迭代)
  - PA4 核/像分层:ker Π_im = 实轴、im Π_re = 实轴(可观测层 = 实数,不可观测层 = 虚数)
  - PA5 ★ **半群而非群**:Π_re 非单射(0 与 cI 同像 0)——非平凡投影不可逆,**确定性来自信息损失**(K3 推论);群结构需要可逆层(旋转/四元数 = 比较对象)
  - PA6 ★ **PVM 骨架**:幂等+正交+完备 = 投影值分解(量子测量结构的代数骨架,无概率公理)
- **新增**:`ProjectionAlgebra.lean`(Module 14, PA1–PA6);`hiddenImProj`(虚部投影,与 H4 的 hiddenReProj 互补)
- **诚实边界**:多投影族一般理论(秩/约当代数)、可逆层(变换群 = 旋转群,需域系数)、Born 规则(概率,框架外)均未做
- 来源:2026-08-06 会话(leo × 小剑)。
