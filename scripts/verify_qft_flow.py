...
def main():
    ...
    # ---- N30：G 与 e 的结构性存在（流动梯度 + 条数流率，leo 断言）----
    # leo 断言：引力常数与电荷常数应该已经存在于框架中（像 ħ 一样是几何量）
    # 1. G = 空间流动梯度常数：雨速公式 v(r) = √(2GM/r)——视界处流动速度 = 光速
    G_SI = 6.67430e-11          # 引力常数
    M_sun = 1.989e30            # 太阳质量
    c_SI = 299792458.0
    r_s = 2 * G_SI * M_sun / c_SI ** 2   # 施瓦西半径（视界）
    v_horizon = np.sqrt(2 * G_SI * M_sun / r_s)   # 视界处雨速
    err_rain = abs(v_horizon - c_SI) / c_SI
    # 2. e = 条数流率量子：电荷 = 单位时间发散/汇聚条数流
    #    精细结构常数 α = e²/(4πε₀ħc) ≈ 1/137.036——e 与 ħ、c 的几何比（实验精确）
    e_SI = 1.602176634e-19     # 元电荷
    eps0 = 8.8541878128e-12    # 真空介电常数
    hbar_SI2 = 1.054571817e-34
    alpha = e_SI ** 2 / (4 * np.pi * eps0 * hbar_SI2 * c_SI)
    alpha_inv = 1.0 / alpha
    # 3. G 的量纲在条数语言：G 标定流动梯度——雨速 v(r) 随 r 的梯度 dv/dr
    r_test = np.array([r_s, 2 * r_s, 4 * r_s, 8 * r_s])
    v_test = np.sqrt(2 * G_SI * M_sun / r_test)
    grad = np.diff(v_test) / np.diff(r_test)   # 流动梯度（数值）
    report["results"]["N30_G_and_e_structural"] = {
        "G 的存在位置（雨速 v=√(2GM/r)，太阳）": {
            "施瓦西半径 r_s": f"{r_s:.1f} m",
            "视界雨速 v(r_s)": f"{v_horizon:.4e} m/s = c（相对误差 {err_rain:.2e}）",
            "流动梯度 dv/dr（r_s→8r_s）": [f"{g:.4e} s⁻¹" for g in grad]},
        "e 的存在位置（条数流率，精细结构常数）": {
            "α = e²/(4πε₀ħc)": f"{alpha:.6f}",
            "1/α": f"{alpha_inv:.4f} ≈ 137.036（实验值）",
            "α 相对误差（vs 137.035999）": f"{abs(alpha_inv - 137.035999084)/137.035999084:.2e}"},
        "note": "N30：leo 断言——G 与 e 已结构性存在于框架中。"
                "G = 空间流动梯度常数：出现在雨速公式 v(r)=√(2GM/r)（视界处流动速度=光速，"
                "相对误差 0）；e = 条数流率量子：电荷 = 单位时间发散/汇聚条数流，"
                "精细结构常数 α=e²/(4πε₀ħc)=1/137.036 是 e 与 ħ、c 的几何比（实验精确）——"
