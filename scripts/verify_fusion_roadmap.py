...
MU_LADDER = [
    ...
    ("G4 D2 锁定判决", 18, 24, 1e-1, "FRC 级约束装置", 72),
...
# 权威口径（门禁）：人·月 = 人数 × 月数（人数 = 各阶段门的人头；月份 = 该阶段月数）
# 曾写错 G4 = 144（隐含 24 人，与团队表 12 人矛盾）——2026-09-15 修正为 72，总量 1233 → 1161
_HEAD_MONTHS = [(2, 3), (3, 3), (5, 6), (8, 6), (12, 6), (20, 12), (28, 12), (35, 12)]
assert [r[5] for r in MU_LADDER] == [n * m for n, m in _HEAD_MONTHS], \
    "阶梯人·月 ≠ 人数 × 月数（人头/月数见 _HEAD_MONTHS）"
assert total_pm == 1161, f"五年总人·月应为 1161，实为 {total_pm}"
R4_ladder = {
...
# 每条工作流的季度人·月分配（人·月 = 人数 × 3 月）
# 用途：把"leo 若自担理论/数值且不计薪"的灵敏度变成**分解出来的数**，而不是拍脑袋的比例
LANE_PM = {
    "Q1": {"理论数值线": 3.0, "诊断线": 2.0, "装置线": 1.0, "对外线": 0.0},
    "Q2": {"理论数值线": 1.5, "诊断线": 4.5, "装置线": 2.5, "对外线": 0.5},
    "Q3": {"理论数值线": 3.0, "诊断线": 7.5, "装置线": 3.0, "对外线": 1.5},
    "Q4": {"理论数值线": 3.0, "诊断线": 6.0, "装置线": 7.5, "对外线": 1.5},
    "Q5": {"理论数值线": 3.0, "诊断线": 6.0, "装置线": 9.0, "对外线": 3.0},
    "Q6": {"理论数值线": 4.5, "诊断线": 6.0, "装置线": 7.5, "对外线": 6.0},
}
for _q in QUARTERS:
    assert abs(sum(LANE_PM[_q[0]].values()) - _q[3]) < 1e-9, \
        f"{_q[0]} 工作流人·月之和 ≠ 季度人·月"
LANE_TOTALS = {lane: sum(LANE_PM[q[0]][lane] for q in QUARTERS) for lane in LANES}
assert abs(sum(LANE_TOTALS.values()) - 93) < 1e-9, "工作流分解总和 ≠ 93"
# 理论/数值线 = 1 FTE × 18 月（leo 现在就是这个角色）
assert abs(LANE_TOTALS["理论数值线"] - 18.0) < 1e-9, "理论/数值线应为 18 人·月（1 FTE × 18 月）"

R6_18months = {
    "总人·月（18 个月）": sum(q[3] for q in QUARTERS),
    "人数曲线": [q[2] for q in QUARTERS],
    "工作流人·月分解": {**LANE_TOTALS,
                        "理论数值线占全部 [%]": round(LANE_TOTALS["理论数值线"] / 93 * 100, 1)},
    "工作流逐季度人·月": LANE_PM,
    "leo 自担理论/数值（不计薪）的灵敏度": {
        "自担人·月": LANE_TOTALS["理论数值线"],
        "依据": "理论/数值线 = 1 FTE × 18 月（分解自季度分配，非拍比例）",
        "纯薪口径省下 [万元]": round(LANE_TOTALS["理论数值线"] * 2.5, 1),
        "含社保口径省下 [万元]": round(LANE_TOTALS["理论数值线"] * 2.5 * 1.35, 1),
        "诚实提醒": "不计薪不是省成本，是把成本从项目账移到个人机会成本；"
                    "对外预算/融资必须按有薪计（否则等于把自己的时间估成 0）"},
...
def main():
    ...
    lines += [
        ...
        "  工作流分解（人·月）："
        + " / ".join(f"{k} {v:g}" for k, v in LANE_TOTALS.items()),
        f"  理论/数值线 {LANE_TOTALS['理论数值线']:g} 人·月 = 1 FTE×18 月"
        f"（占 {R6_18months['工作流人·月分解']['理论数值线占全部 [%]']}%）"
        f"——leo 若自担且不计薪，项目侧省该额度",
        "  逐季度：",
    ]
