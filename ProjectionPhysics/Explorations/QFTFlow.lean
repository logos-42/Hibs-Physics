...
/-! ### GQP★. 光子：激发电子螺旋 ⟹ 无质量，随流以光速运动

leo（2026-08-16）假设（光子结构）：
- ★ 光子 = 加速运动的负电荷产生反引力场，使电子质量和电荷消失的激发态
- ★ 粒子性：光子由电子激发变成（电子实体遗留的螺旋结构）
- ★ 波动性：空间本身时刻波动 + 叠加圆柱状螺旋运动，波动速度 = 光速
- ★ 光子静止在空间中，随空间一同运动（GQC3 流动携带：v = C，
  等效速度 = 光速；QFT1：完全随流 ⟹ dτ² = 0 = 非激发）
- 模型 A：一个激发电子，圆柱状螺旋运动，直线部分 = 光速
  （单扭量 ⟹ TW1：det(π⊗π̄) = 0，无质量必然）
- 模型 B：两个激发电子绕同一轴线对称旋转 + 沿旋转平面垂直方向
  直线以光速运动——★ 环向动量对称抵消 ⟹ 净动量纯轴向 ⟹ 无质量
  （两模型共享光锥 E² = |p|²）
- 诚实：代数恒等（真但平凡）+ 波动性为数学骨架（需波动方程）；解释层。 -/

/- GQP1. 光子模型 A 无质量——TW1 已有（det(π⊗π̄) = 0，秩 1）：
   单激发电子螺旋 = 单扭量，无质量必然。引用 QFT9 统一命题
   （单扭量 det = 0 = 非激发 = 无质量）。 -/

/-- GQP2★. 模型 B 环向抵消：两激发电子绕同一轴线对称旋转
    （环向动量 +py 与 −py），总动量环向分量 = 0——净动量纯轴向
    （直线光速部分）。证明：py + (−py) = 0（ring）。 -/
lemma gqp2_antipodal_cancel (py : ℂ) : py + (-py) = 0 := by ring

/-- GQP2b★. 无质量判据（类光）：总动量 (E, p_x, 0, p_z) 满足
    E² = p_x² + p_z²——环向抵消后无质量条件与单螺旋相同
    （光子两种模型共享同一光锥）。证明：rw + ring。 -/
lemma gqp2b_massless_axial (E px pz : ℝ) (hmass : E^2 = px^2 + pz^2) :
    E^2 - (px^2 + 0^2 + pz^2) = 0 := by
  rw [hmass]
  ring

/-- GQP3★. 光子随流：激发电子完全随空间流动（v = C）且质量消失（m = 0）
    ⟹ 相对流动动量 p = m(C − v) = 0——光子静止在空间中，随空间一同运动
    （GQC3 流动携带：等效速度 = 光速；GQF1 流动动量公式）。 -/
lemma gqp3_photon_carried (m C v : ℂ) (hm : m = 0) (hv : C = v) :
    m * (C - v) = 0 := by
  rw [hm, hv]
  ring

/- GQP4（数学骨架，诚实标注）：波动性 = 空间本身的波动。
   - 空间时刻波动 + 叠加圆柱状螺旋运动，波动速度 = 光速
   - 光子随空间波动传播——波动方程（∂²φ/∂t² = c²∇²φ）的格点版留作后续；
     已有 QFT1（非激发 = 完全随流 dτ² = 0）与 GQC2（格点光锥）作为
     波动传播的因果骨架
   - 数值：N26 光子结构图（模型 A 单螺旋 vs 模型 B 双螺旋对称）。 -/

/-! ### GQR★. 光子能量匹配：E = ħω = 螺旋旋转能量（数条数第一步）

leo（2026-08-16）匹配尝试：普朗克常数 = 空间旋转条数在扭量聚合下的频率效果。
- ★ 螺旋圈数 = 频率：每秒圈数 f = ω/2π（**可数**——数条数第一步）
- ★ 每圈角动量 = ħ：光子自旋 = ħ（标准物理实验事实）⟹ 螺旋每圈携带角动量 ħ
- ★ E = ħω = 每圈角动量 × 角速度 = 螺旋旋转能量（旋转能量恒等 E = L·ω）
- ★ 普朗克-爱因斯坦关系涌现：E = ħω = (h/2π)·(2πf) = h·f——
  **E = 每圈角动量 h × 每秒圈数 f**（条数 = 圈数 = 频率，可数）
- 匹配判定：光量子关系从螺旋几何涌现（光子能量 = 螺旋旋转能量）；
  ħ 的条数解释 = 每弧度角动量（可数几何量，非神秘常数）
- 诚实：E = Lω 与 E = h·f 是经典/量子恒等（真但平凡）；L = ħ 依赖
  光子自旋实验事实；若未来能从螺旋几何参数独立数出 ħ 数值才是预言。 -/

/-- GQR1. 螺旋运动学：轴向直线速度 = 光速（leo 假设"直线运动部分是光速"）——
    螺旋轴向位移 = c·t，环向 = 空间旋转（无物质载荷，GQC3 流动携带）。 -/
lemma gqr1_helix_axial (c t : ℝ) : c * t = c * t := rfl

/-- GQR2★. 旋转能量恒等：E = ½L·ω——螺旋旋转能量 = 角动量 × 角速度。
    经典版：L = mr²ω（半径 r 质量 m），E = ½mr²ω² = ½(mr²ω)·ω。 -/
lemma gqr2_rotational_energy (m r ω : ℝ) :
    (1 / 2 : ℝ) * (m * r ^ 2 * ω ^ 2) = (1 / 2 : ℝ) * ((m * r ^ 2 * ω) * ω) := by ring

/-- GQR3★. 普朗克-爱因斯坦关系涌现：E = ħω = h·f——
    ħ = h/2π（每弧度角动量），ω = 2πf（每秒圈数 × 每圈 2π 弧度），
    ⟹ E = 每圈角动量 h × 每秒圈数 f。条数 = 圈数 = 频率（可数）。
    证明：ring_nf（h/(2π) · 2πf = h·f）。 -/
lemma gqr3_planck_relation (h f : ℝ) :
    (h / (2 * Real.pi)) * (2 * Real.pi * f) = h * f := by
  field_simp [Real.pi_ne_zero]

/-- GQR3b★. 光子能量 = 螺旋旋转能量：E = ħω（每圈角动量 ħ × 角速度 ω）。
