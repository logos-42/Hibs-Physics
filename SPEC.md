...
| MC1 | `anchor_mass_nonzero_of_internal_motion` + `anchor_mass_zero_of_no_internal_motion` | **内部运动状态 s（自旋/内部关系）→ 空间运动 F → 质量 m := 锚定效果（旋量阻抗）→ 内部运动非零 ⟹ m ≠ 0；静止流 ⟹ m = 0** | ★ **最小核心命题（锚定版）**：质量=物质抵抗空间本身运动的锚定效果，非场的激发能量（`MinimalCore.lean`） |
| MC2 | `triplet_glueball_mass_squared_formula` + `triplet_glueball_mass_squared_positive_of_any_nonzero` | **G=(a,b,c), color profile=(1,1,1), m_G²=|a|²+|b|²+|c|²；至少一个非零 ⟹ m_G² > 0** | ★ **胶球最小版**：三胶子内部运动状态色单态的锚定质量平方（`MinimalCore.lean`） |
