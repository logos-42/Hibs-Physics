...
\subsection{信息 = 辛体积、光子螺旋与"数条数"（2026-08-16）}

一条快速推进的探索线（形式化于
\texttt{ProjectionPhysics/Explorations/QFTFlow.lean}，4146 jobs，零
`sorry`；数值检查 N1--N28 全部机器精度）沿四个方向扩展框架。

\textbf{信息 = 辛关联体积。} 酉传输（流动传播）保持
$\det(AA^\dagger)$（GQC1）：信息——定义为扭量的辛关联体积——随流
守恒。因果性 = 格点局域性（GQC2）：最近邻跳跃矩阵 $t$ 步后带宽 $\le t$，
传播核在格点光锥外为零。非均匀流动中等效速度 $= 1+v_{\rm flow}$
（GQC3）：超光速\textbf{等效}运动是几何（坐标拖曳）效应——
Alcubierre/Painlev\'e--Gullstrand 结构——而信号从不超越局部光速。
不声称新物理；这些属于解释层。

\textbf{流动动量与四力。} 动量假设 $p = m(C-v)$（$C$ = 矢量光速，
$v$ = 物体速度）下，莱布尼茨法则给出（GQF）
\[
\frac{d}{dt}\big[m(C-v)\big] = \dot m\,C + m\dot C - \dot m\,v - m\dot v,
\]
四个通道对应电场力（$\dot m C$）、核力（$m\dot C$）、磁场力
（$\dot m v$）与万有引力/惯性力（$m\dot v$）。这是代数恒等；
耦合常数未推导。

\textbf{光子 = 激发电子螺旋。} 光子是质量与电荷消失的激发电子；
螺旋（模型 B：双螺旋对称）环向动量抵消，净动量纯轴向，
$E^2=|p|^2$（无质量）——两模型共享光锥。
...
\]
被重新解释为\textbf{每圈角动量 $\times$ 每秒圈数}（GQR）：
$\hbar$ 变成每弧度角动量——一个可数的几何量——圈数即（可数的）
频率。数值检验精确（相对误差 $0$）。胶球规则 $m=\sqrt{N}\,M_0$
同样解读：$N=3$ 条（三个锚定方向）；色 $=3$ 是同一个"3"
（三个空间方向）；$8$ 胶子 $= 2^3 = \dim C\ell(6)$（TW5）。
诚实边界：夸克质量谱（$2.2$\,MeV $\to$ $173$\,GeV）无条数规律
（QCD 自由参数）；$L=h$ 仍依赖实测光子自旋；从螺旋几何第一性
推导 $\hbar$ 是开放的下一步。

\textbf{圆周 = 波长。} $h$ 与 $\hbar$ 之间的 $2\pi$ 因子正是螺旋
截面的圆周。在几何选择 $r=\lambda/2\pi$（约化波长 = 螺旋半径）下
推导链闭合：$2\pi r = \lambda$（一圈 = 一个波长）；
$\hbar = r\,p$（角动量 = 半径 $\times$ 动量，经典公式）；
于是德布罗意关系 $p = h/\lambda$ 与普朗克-爱因斯坦关系
$E = pc = hf$ \emph{从螺旋几何涌现}。代入物理常数（500\,nm 光），
$\hbar = r p$ 复现实测 $\hbar$ 到 $6\times10^{-10}$（浮点精度），
$E = pc$ 与 $E = hf$ 精确一致。诚实状态：$r=\lambda/2\pi$ 是来源
未知的几何假设，$\hbar = rp$ 是经典恒等；推导链已闭合，第一性
推导尚未达成。
