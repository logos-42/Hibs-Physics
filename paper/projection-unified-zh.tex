...
\title{流动空间的大一统：\\ 质量、信息、因果、力与量子关系的几何基础}

\author{刘元杰}
\affiliation{独立研究者，中国杭州}
...
\end{align}
以物理常数代入机器精度验证（图~\ref{fig:circ}）：$\lambda=500$\,nm 时
$r=\lambda/2\pi=79.58$\,nm，$p=h/\lambda=1.3252\times10^{-27}$\,kg\,m\,s$^{-1}$，
$\hbar=rp=1.0546\times10^{-34}$\,J\,s 复现实测 $\hbar$ 到
$6\times10^{-10}$；$E=pc$ 与 $E=hf$ 精确一致。

\begin{figure}[htbp]
\includegraphics[width=0.9\columnwidth]{figures/fig_circumference_wavelength.png}
\caption{圆周 = 波长：螺旋截面半径 $r=\lambda/2\pi$（约化波长）；圆周
$2\pi r=\lambda$。推导链 $r=\lambda/2\pi \Rightarrow \hbar=rp \Rightarrow
p=h/\lambda \Rightarrow E=hf$ 闭合。}
\label{fig:circ}
\end{figure}
...
未知）；(ii) 剩余输入常数（$e$、$M_0$、$G$——"第二输入缺口"）的推导；
(iii) 标准框架之外的任何新的可证伪预言（格点 $N$ 序列是后验拟合）；
(iv) 光子波动性的连续波动方程、电荷散度形式仍是形式化骨架。
