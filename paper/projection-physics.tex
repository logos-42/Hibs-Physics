...
\textbf{Circumference = wavelength.}  The factor $2\pi$ between $h$ and
$\hbar$ is the circumference of the helix cross-section.  With the
geometric choice $r=\lambda/2\pi$ (the reduced wavelength as helix radius),
the chain closes: $2\pi r = \lambda$ (one turn = one wavelength);
$\hbar = r\,p$ (angular momentum = radius $\times$ momentum, classical);
hence the de~Broglie relation $p = h/\lambda$ and the Planck--Einstein
relation $E = pc = hf$ \emph{emerge from the helix geometry}.  Substituting
physical constants (500\,nm light), $\hbar = r p$ reproduces the measured
$\hbar$ to $6\times10^{-10}$ (float precision) and $E = pc$ matches
$E = hf$ exactly.  Honest status: $r=\lambda/2\pi$ is a geometric
assumption whose origin is unknown, and $\hbar = rp$ is a classical
identity; the chain is closed but the first-principle derivation is not
yet achieved.
